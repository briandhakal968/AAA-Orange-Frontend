...
export default function EditProductPage() {
  ...
  const handleMediaSelect = (url: string) => {
    if (mediaPickerTarget === "main") {
      ...
      setExistingGallery(prev => [...prev, url]);
    }
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [productData, countriesData, brandsData, categoriesData, attributesData] = await Promise.all([
          adminApi.get<any>("/products/" + productSlug),
        ...
        const allCategoryIds = productData.categories 
          ? productData.categories.map((c: any) => c.id)
          : [];
        if (productData.category_id && !allCategoryIds.includes(productData.category_id)) {
          allCategoryIds.push(productData.category_id);
        }
        setFormData({
          ...
          brand_id: productData.brand_id ? productData.brand_id.toString() : "",
          sku: productData.sku || "",
          minimum_stock: productData.minimum_stock ?? 0,
          damaged_stock: productData.damaged_stock ?? 0,
        });

        setMainImageUrl(productData.image || "");
        setExistingGallery(productData.gallery ? productData.gallery.map((g: any) => g.image) : []);

        const existingPrices = productData.prices || [];
        setCountryPrices(countriesData.map((c: Country) => {
          const existing = existingPrices.find((p: any) => p.country_id === c.id);
          return {
            country_id: c.id,
        ...
        if (productData.attributes && attributesData) {
          productData.attributes.forEach((attr: any) => {
            ...
            const attrData = attributesData.find((a: any) => a.id === attrId);
            const valueExists = attrData?.values?.some((v: any) => v.value === attrValue);
            if (attrValue && valueExists && !attrMap[attrId].includes(attrValue)) {
              attrMap[attrId].push(attrValue);
            }
          });
        }
        setSelectedAttributes(attrMap);

        // Load additional info from product
        if (productData.additionalInfo && Array.isArray(productData.additionalInfo)) {
          const loadedInfo = (productData.additionalInfo as any[])
            .filter((info: any) => info && (info.label || info.value))
            .map((info: any) => ({
              label: info.label || "",
              value: info.value || ""
            }));
          if (loadedInfo.length > 0) {
            setAdditionalInfo(loadedInfo);
          }
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!productSlug) {
      alert("Product slug not found. Please refresh the page.");
      setSaving(false);
      return;
    }
...
attributes: Object.entries(selectedAttributes).flatMap(([attrId, values]) =>
          values.map(value => ({ attribute_id: parseInt(attrId), value: value.toString() }))
        ),
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <p className="text-xs text-slate-500 mt-3">Click "Update Product" button to save additional information</p>
          </div>
