...
interface AdminTodaysDealsResponse {
  products: ProductItem[];
  selected_ids: number[];
}

export default function TodaysDealsAdminPage() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const data = await adminApi.get<AdminTodaysDealsResponse>("/admin/todays-deals");
  ...
  const handleSave = async () => {
    ...
    setSuccess("");
    try {
      await adminApi.post("/admin/todays-deals", { product_ids: selectedIds });
      setSuccess("Today's Deals saved successfully!");
      setTimeout(() => setSuccess(""), 3000);
    } catch (err) {
      console.error("Error saving today's deals:", err);
      setError("Failed to save Today's Deals.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Today's Deals</h1>
          <p className="text-sm text-slate-500 mt-1">
            Select products to display on the Today's Deals page ({selectedCount} selected)
          </p>
        </div>
        <button
          onClick={handleSave}
